...
```{r}
## We can compare the apache score to the score where there is a 99% chance that the patient will pass away within 48 hours 
...
p2 <- ggplot(data = df_apache, aes(y = Value, x = Apache)) + 
  ...
  scale_y_continuous(limits = c(0, 75), expand = c(0, 5)) +
  theme(panel.background = element_blank(), 
        axis.line.x = element_line(colour = "grey40", size = .2), 
        axis.line.y = element_line(colour = "grey40", size = .2),
